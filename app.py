...
<style>
    ...
    /* Sidebar */
    [data-testid="stSidebar"] { background: linear-gradient(180deg,#211b56 0%,#312f7e 100%); }
    [data-testid="stSidebar"] h2,
    [data-testid="stSidebar"] h3,
    [data-testid="stSidebar"] h4,
    [data-testid="stSidebar"] p,
    [data-testid="stSidebar"] li,
    [data-testid="stSidebar"] code { color: #f8fafc !important; }
...
<div class="api-note">Create a key in Google AI Studio and paste it below to update only Gemini.</div>
</div>""",
            unsafe_allow_html=True,
        )
        with st.form("google_key_form", clear_on_submit=True):
            google_key = st.text_input(
                ...
                type="password",
                value="",
                placeholder="Paste a new Google AI key, or leave blank to test saved key",
            )
            save_google, test_google = st.columns(2)
            save_google_clicked = save_google.form_submit_button("Save Google Key", use_container_width=True)
            test_google_clicked = test_google.form_submit_button("Test Google AI", use_container_width=True)
            if save_google_clicked:
                saved = save_env_values({"GOOGLE_API_KEY": google_key.strip()})
                if saved:
                    st.success("Google AI key saved.")
                else:
                    st.warning("Paste a Google AI key before saving.")
            if test_google_clicked:
                try:
                    key = google_key.strip() or os.getenv("GOOGLE_API_KEY")
                    if not key:
                        raise ValueError("Enter a Google AI key or save one first.")
                    ensure_event_loop()
                    ...
                    ensure_event_loop()
                    res = llm.invoke("Say 'Gemini OK' only.")
                    st.success(f"Gemini working: {res.content[:40]}")
                except Exception as e:
